...
public class PointSemiVariogram_GridIndex extends RGIS implements PointSpatialStatistic {
	
	private GISPointGridIndex<PumsHouseholdRealization>[] maps;
	...
	private PointSemiVariogram_GridIndex mapSVs[];

	/**
	 * Construct blank instance.
	 */
	protected PointSemiVariogram_GridIndex() {
	}
     ...
     */
    public static PointSemiVariogram_GridIndex create(double lagDistance, double maxDistance, TransformAttributes2double transformObject) {
    	PointSemiVariogram_GridIndex psv = new PointSemiVariogram_GridIndex();
        psv.initVars(lagDistance, maxDistance, transformObject);
        psv.initialize((GISPointGridIndex<PumsHouseholdRealization>[]) null);
        return psv;
    }
     ...
     * @return
     */
    public static PointSemiVariogram_GridIndex createStat(double lagDistance, double maximumDistance,
    		GISPointGridIndex<PumsHouseholdRealization> map,
     ...
     */
    public static PointSemiVariogram_GridIndex createStat(double lagDistance, double maximumDistance,
    		GISPointGridIndex<PumsHouseholdRealization>[] maps,
            TransformAttributes2double transformObject) {
    	PointSemiVariogram_GridIndex psv = new PointSemiVariogram_GridIndex();
        psv.initVars(lagDistance, maximumDistance, transformObject);
        psv.initialize(maps);
    ...
    public void initialize(GISPointGridIndex<PumsHouseholdRealization> dataMap) {
        initialize(new GISPointGridIndex[] { dataMap });
    }
    
    /**
     * Register a set of maps, and update statistics accordingly.
     */
    public void initialize(GISPointGridIndex<PumsHouseholdRealization>[] dataMaps) {
        this.maps = dataMaps;
        this.mapSVs = null;
        ...
        if(maps != null) {
            this.mapSVs = new PointSemiVariogram_GridIndex[maps.length];
            for (int m = 0; m < maps.length; m++) {
                mapSVs[m] = PointSemiVariogram_GridIndex.create(lagDist, maxDist, transformObject);
                calcCov(maps[m], m);
            }
        }
    }
     ...
     * @param mapNumber
     */
    private void calcCov(GISPointGridIndex<PumsHouseholdRealization> qt, int mapNumber) {
     ...
     * PointSemiVariogram.
     */
    public PointSemiVariogram_GridIndex createCopy() {
    	PointSemiVariogram_GridIndex newC = new PointSemiVariogram_GridIndex();
        newC.copy(this);
        return newC;
    }
     ...
     */
    public void copy(PointSemiVariogram_GridIndex c) {
        maps = c.maps;
        ...
        if(c.mapSVs == null)
            ...
            this.mapSVs = new PointSemiVariogram_GridIndex[c.mapSVs.length];
    ...
    public void modifySS4NewPt(PumsHouseholdRealization newPoint, int mapNumber) {
        ...
        GISPointGridIndex<PumsHouseholdRealization> map = maps[mapNumber];
    ...
    public double spread(SpatialStatistic goalSpatialStatistic) {
        return spread((PointSemiVariogram_GridIndex) goalSpatialStatistic);
    }
    
    /**
     * Returns spread between two PointSemiVariograms.
     */
    public double spread(PointSemiVariogram_GridIndex goal) {
        int lag;
        double dist;
        double sumLSA = 0.0;
     ...
     */
    public void printReduction(PrintStream out, PointSemiVariogram_GridIndex origPsv) {
    	PointSemiVariogram_GridIndex curPsv = this;
        out.format("%s:\n", curPsv.toString());
     ...
     */
    public void printComparison(PrintStream out, PointSemiVariogram_GridIndex origPsv, PointSemiVariogram_GridIndex goalPsv) {
    	PointSemiVariogram_GridIndex curPsv = this;
        out.format("%s:\n", curPsv.toString());
     ...
     * @throws IOException
     */
    public PointSemiVariogram_GridIndex(String fName) throws IOException {
        calculated = true;
        String longFN = fName;
        ...
        PointSemiVariogram_GridIndex oldDG = PointSemiVariogram_GridIndex.create(lagDist, maxDist, null);
        ...
        while (s != null && s.length() > 0) {
            ...
            oldDG = PointSemiVariogram_GridIndex.create(lagDist, maxDist, null);
            oldDG.copy(this);
