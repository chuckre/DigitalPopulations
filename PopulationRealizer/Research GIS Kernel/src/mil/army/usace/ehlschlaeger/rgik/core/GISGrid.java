 ...
 */
public class GISGrid extends GISData implements Serializable, Cloneable {
    /** Flag for GRASS v.4 ASCII files. */
    public final static int GRASS4 = 1;
    /** Flag for ESRI ASCII files. */
    public final static int ESRI = 2;
	...
	public boolean equalsGrid(GISGrid otherGrid) {
        ...
        if (equ) {
            equ = (NSRes == otherGrid.NSRes && EWRes == otherGrid.EWRes
                    && numCols == otherGrid.numCols && numRows == otherGrid.numRows);
        }
        return equ;
    }
	...
	public static GISGrid loadGRASS4(String filename) throws IOException {
        ...
        String s;
        // The GRASS5 header now can have entries: multiplier, null, and type.
        // If type is not set in the header, the new map is created integer if
        // all of the values in the input file are integer, and float otherwise.
	 ...
	 */
	public GISGrid( double westEdge, double northEdge, double EWResolution,
			double NSResolution, int numberRows, int numberColumns) {
		super( westEdge, northEdge, westEdge + EWResolution * numberColumns,
			northEdge - NSResolution * numberRows);
		EWRes = EWResolution;
		NSRes = NSResolution;
		numRows = numberRows;
	...
	public GISGrid( GISGrid existingGrid) {
	    this(existingGrid.getWestEdge(), existingGrid.getNorthEdge(),
	        existingGrid.getEWResolution(), existingGrid.getNSResolution(),
	        existingGrid.getNumberRows(), existingGrid.getNumberColumns());
	...
	public double distance( int rowA, int colA, int rowB, int colB) {
		return( (double) Math.sqrt( (double) 
			(( colA - colB) * EWRes * ( colA - colB) * EWRes +
			 ( rowA - rowB) * NSRes * ( rowA - rowB) * NSRes)
		));
	}
	...
	public GISGrid getBoundingGrid() {
		...
		GISGrid newGrid = new GISGrid( newWestEdge, newNorthEdge, getEWResolution(),
			getNSResolution(), newRows, newCols);
		return newGrid;
	}
	...
	public double getCellCenterNorthing( int cellRow, int cellColumn) {
		//System.out.println( "n:" + getNorthEdge() + ", cellRow:" + cellRow + ", NSRes:" + getNSResolution());
    ...
    public int getExactRowIndex(double northing) {
        ...
        if(Math.abs(Math.round(row)-row) > EPSILON)
            throw new DataException("Northing value "+northing+" does not lie on a lattice line");
        return (int)Math.round(row);
    }
    
    public int getExactColumnIndex(double easting) {
        double col = (easting - getWestEdge()) / getEWResolution();
        if(Math.abs(Math.round(col)-col) > EPSILON)
            throw new DataException("Easting value "+easting+" does not lie on a lattice line");
    ...
    public void writeSaTScanGridFile(String filename) throws IOException {
        PrintWriter out = new PrintWriter(
              new BufferedWriter(new FileWriter(filename + ".grd")));
	 ...
	 */
	public void print() {
		System.out.print( "w: " + getWestEdge() + ", e: " + getEastEdge() + 
			", s: " + getSouthEdge() + ", n: " + getNorthEdge());
...
//	protected void printEsriHeader( PrintWriter out) {
//		out.println( "ncols         " + getNumberColumns());
//		out.println( "nrows         " + getNumberRows());
//		out.println( "xllcorner     " + getWestEdge());
//		out.println( "yllcorner     " + getSouthEdge());
//		out.println( "cellsize      " + getEWResolution());
//	}
	...
	public GISGrid[] segmentGrid( int rowsPerSegment, int columnsPerSegment, int cellOverLap) {
		...
		for( int r = 0; r < segmentRows; r++) {
			...
			for( int c = 0; c < segmentColumns; c++) {
				...
				int maxColumn = (int) Math.min( getNumberColumns() - 1, (c + 1) * 
					columnsPerSegment + cellOverLap - 1);
				double westEdge = getWestEdge() + minColumn * getEWResolution();
				segments[ doneSegments] = new GISGrid( westEdge, northEdge, getEWResolution(),
					getNSResolution(), maxRow - minRow + 1, maxColumn - minColumn + 1);
				doneSegments++;
			}
		}
		return segments;
	...
	/**
	 * Set all no-data flags to one value.
	 * 
	 * @param value
	 */
    ...
    public static GISGrid loadEsriAscii(MyReader fr) throws IOException {
        ...
        s = fr.readLine().trim();
        if(! s.startsWith(k))
            throw new DataException(String.format("Bad map file: expecting \"%s <number>\", found \"%s\"", k, s));
	...
	public void writeAsciiEsri( String fileName) throws IOException {
        ...
        PrintWriter out = new PrintWriter(
				new BufferedWriter( new FileWriter( ESRI_ASCII.findFile(fileName))));
		out.println( "ncols         " + getNumberColumns());
		out.println( "nrows         " + getNumberRows());
