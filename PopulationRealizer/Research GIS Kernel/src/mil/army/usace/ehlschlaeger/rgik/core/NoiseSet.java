...
public class NoiseSet extends RGIS {
	...
	public NoiseSet( Sensor[] sensors, double[] temperatures, 
			double[] temperatureTimes, double maximumToleranceInMeters) {
		super();
        assert sensors != null;
...
//System.out.print( "(" + earliestSensor + "," + earliestEvent + ") at " + earliestTime + ", ");
...
//System.out.println();
		...
		for( int n = 0; n < noisesDone; n++) {
			// NoiseEvent( double[] x, double[] y, double[] t, double temperatureCelcius)
			// find four noises that might be an event and build NoiseEvent to find fitness
			int homeSensor = timeOrderSensor[ n];
			lowPossibleNoise[ homeSensor] = timeOrderEvent[ n];
			hihPossibleNoise[ homeSensor] = timeOrderEvent[ n];
			for( int s = 0; s < sensors.length; s++) {
				if( s != homeSensor) {
					findHighLow( homeSensor, s, timeOrderEvent[ n],
						lowPossibleNoise, hihPossibleNoise);
				}
			}
			int sCount = 0;
			for( int s = 0; s < sensors.length; s++) {
...
/*
System.out.println( "Sensor[" + s + "] low: " + lowPossibleNoise[ s] + " at " + 
sensors[ s].getEvent( lowPossibleNoise[ s]) + ", high: " + hihPossibleNoise[ s] + " at " +
sensors[ s].getEvent( hihPossibleNoise[ s]));
*/
					sCount++;
				}
			}
...
//System.out.println( "Sensor[" + s + "] (" + x[i] + "," + y[i] + "," + t[i] + ")");
							i++;
						}
					}
					NoiseEvent ne = new NoiseEvent( x, y, t, getTemperatureAtTime( t[ 0]));
					ne.solve();
//System.out.println( "solX: " + ne.getSolutionX() + ", solY: " + ne.getSolutionY());
					double[] error = new double[ sCount];
					double aveErrorInMeters = ne.distanceFitness( error);
/* 
System.out.println( "ave error: " + aveErrorInMeters);
for( int ii = 0; ii < error.length; ii++) {
	System.out.print( " [" + ii + "]:" + error[ ii]);
}
System.out.println();
*/
					// find distance from centroid of sensors to solution
					...
					// find time differencial
					double tDif = ne.getMinimumTimeDifference() - ne.getSolutionTime();
					if( aveErrorInMeters <= mTolerance &&
					    mTimeDiffercial >= tDif &&
					    mDistFromSensors * mDistFromSensors  >= solutionDistanceSquared) {
...
//System.out.println( "adjusting array sizes");
							NoiseEvent[] oldPossibleSolutions = possibleSolutions;
	...
	private boolean findHighLow( int homeSensor, int checkSensor, int homeSensorNoiseEvent,
			int[] lowPossibleNoise, int[] hihPossibleNoise) {
		lowPossibleNoise[ checkSensor] = sensors[ checkSensor].getNumberEvents();
		hihPossibleNoise[ checkSensor] = -1;
		int e = 0;
		while( e < sensors[ checkSensor].getNumberEvents() && 
			sensors[ checkSensor].getEvent( e) < sensors[ homeSensor].getEvent( homeSensorNoiseEvent)) {
			e++;
		}
		if( e == sensors[ checkSensor].getNumberEvents())
			return false;
		boolean possible = false;
		...
		for( ; e < sensors[ checkSensor].getNumberEvents(); e++) {
			...
			double timeBetweenSensors = sensorDistances[ homeSensor][ checkSensor] / 
				getSpeedOfSoundAtTime( (homeSensorTime + checkSensorTime) / 2.0);
				//System.out.println( "hT: " + homeSensorTime + ", c: " + checkSensorTime + ", btwn: " + timeBetweenSensors);
			if( homeSensorTime + timeBetweenSensors < checkSensorTime) {
				//hihPossibleNoise[ checkSensor] = e - 1;
	  ...
	  for( int sen = 6; sen <= 6; sen++) {
		...
		*/
		...
		if( numberSolutions == 0) {
			...
			//System.out.println( numberSolutions + " solutions");
			NoiseEvent[] events = ns.getPossibleSolutions();
			//double[] aveErrors = ns.getAverageErrorOfSolutions();
